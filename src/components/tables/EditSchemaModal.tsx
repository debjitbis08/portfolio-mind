...
export default function EditSchemaModal(props: EditSchemaModalProps) {
  const [name, setName] = createSignal(props.table.name);
  const [columns, setColumns] = createSignal<ColumnDefinition[]>(
    JSON.parse(JSON.stringify(props.table.columns)) // Deep clone
  );
  const [saving, setSaving] = createSignal(false);
  const [error, setError] = createSignal("");

  // Track the next column ID
  let nextColId =
    Math.max(
  ...
  const removeColumn = (id: string) => {
    ...
    if (
      !confirm("Delete this column? Existing data in this column will be lost.")
    )
      return;
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      ...
      const response = await fetch(`/api/tables/${props.table.id}`, {
        method: "PUT",
  ...
  return (
    <div
      class="fixed inset-0 z-[70] flex items-center justify-center bg-crust/80 backdrop-blur-sm p-4"
      onClick={(e) => {
    ...
    >
      <div class="bg-base border border-surface1 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        ...
        <div class="flex items-center justify-between p-4 border-b border-surface1">
          <h3 class="text-lg font-medium text-text">Edit Table Schema</h3>
          <button
